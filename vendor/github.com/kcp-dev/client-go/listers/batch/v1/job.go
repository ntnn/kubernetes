...
func (l *jobClusterLister) Cluster(clusterName logicalcluster.Name) listersbatchv1.JobLister {
	return &jobLister{
		kcplisters.New[*batchv1.Job](l.indexer, clusterName, batchv1.Resource("job")),
		l.indexer,
		clusterName,
	}
}
...
func (l *jobLister) Jobs(namespace string) listersbatchv1.JobNamespaceLister {
	return newJobNamespaceLister(l.ResourceIndexer, namespace)
}
...
// newJobNamespaceLister returns a new listersbatchv1.JobNamespaceLister.
func newJobNamespaceLister(indexer kcplisters.ResourceIndexer[*batchv1.Job], namespace string) listersbatchv1.JobNamespaceLister {
	return &jobNamespaceLister{
		kcplisters.NewNamespaced(indexer, namespace),
	}
}

// NewJobLister returns a new listersbatchv1.JobLister.
// We assume that the indexer:
// - is fed by a workspace-scoped LIST+WATCH
// - uses cache.MetaNamespaceKeyFunc as the key function
// - has the cache.NamespaceIndex as an index
func NewJobLister(indexer cache.Indexer) listersbatchv1.JobLister {
	return &jobScopedLister{
		listers.New[*batchv1.Job](indexer, batchv1.Resource("job")),
		indexer,
	}
}
...
// Jobs returns an object that can list and get Jobs in one namespace.
func (l *jobScopedLister) Jobs(namespace string) listersbatchv1.JobNamespaceLister {
