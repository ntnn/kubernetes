...
func (l *serviceClusterLister) Cluster(clusterName logicalcluster.Name) listerscorev1.ServiceLister {
	return &serviceLister{
		kcplisters.New[*corev1.Service](l.indexer, clusterName, corev1.Resource("service")),
		l.indexer,
		clusterName,
	}
}
...
func (l *serviceLister) Services(namespace string) listerscorev1.ServiceNamespaceLister {
	return newServiceNamespaceLister(l.ResourceIndexer, namespace)
}
...
// newServiceNamespaceLister returns a new listerscorev1.ServiceNamespaceLister.
func newServiceNamespaceLister(indexer kcplisters.ResourceIndexer[*corev1.Service], namespace string) listerscorev1.ServiceNamespaceLister {
	return &serviceNamespaceLister{
		kcplisters.NewNamespaced(indexer, namespace),
	}
}

// NewServiceLister returns a new listerscorev1.ServiceLister.
// We assume that the indexer:
// - is fed by a workspace-scoped LIST+WATCH
// - uses cache.MetaNamespaceKeyFunc as the key function
// - has the cache.NamespaceIndex as an index
func NewServiceLister(indexer cache.Indexer) listerscorev1.ServiceLister {
	return &serviceScopedLister{
		listers.New[*corev1.Service](indexer, corev1.Resource("service")),
		indexer,
	}
}
...
// Services returns an object that can list and get Services in one namespace.
func (l *serviceScopedLister) Services(namespace string) listerscorev1.ServiceNamespaceLister {
