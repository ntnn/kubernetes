...
func (l *podClusterLister) Cluster(clusterName logicalcluster.Name) listerscorev1.PodLister {
	return &podLister{
		kcplisters.New[*corev1.Pod](l.indexer, clusterName, corev1.Resource("pod")),
		l.indexer,
		clusterName,
	}
}
...
func (l *podLister) Pods(namespace string) listerscorev1.PodNamespaceLister {
	return newPodNamespaceLister(l.ResourceIndexer, namespace)
}
...
// newPodNamespaceLister returns a new listerscorev1.PodNamespaceLister.
func newPodNamespaceLister(indexer kcplisters.ResourceIndexer[*corev1.Pod], namespace string) listerscorev1.PodNamespaceLister {
	return &podNamespaceLister{
		kcplisters.NewNamespaced(indexer, namespace),
	}
}

// NewPodLister returns a new listerscorev1.PodLister.
// We assume that the indexer:
// - is fed by a workspace-scoped LIST+WATCH
// - uses cache.MetaNamespaceKeyFunc as the key function
// - has the cache.NamespaceIndex as an index
func NewPodLister(indexer cache.Indexer) listerscorev1.PodLister {
	return &podScopedLister{
		listers.New[*corev1.Pod](indexer, corev1.Resource("pod")),
		indexer,
	}
}
...
// Pods returns an object that can list and get Pods in one namespace.
func (l *podScopedLister) Pods(namespace string) listerscorev1.PodNamespaceLister {
