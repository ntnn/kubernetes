...
func (l *limitRangeClusterLister) Cluster(clusterName logicalcluster.Name) listerscorev1.LimitRangeLister {
	return &limitRangeLister{
		kcplisters.New[*corev1.LimitRange](l.indexer, clusterName, corev1.Resource("limitrange")),
		l.indexer,
		clusterName,
	}
}
...
func (l *limitRangeLister) LimitRanges(namespace string) listerscorev1.LimitRangeNamespaceLister {
	return newLimitRangeNamespaceLister(l.ResourceIndexer, namespace)
}
...
// newLimitRangeNamespaceLister returns a new listerscorev1.LimitRangeNamespaceLister.
func newLimitRangeNamespaceLister(indexer kcplisters.ResourceIndexer[*corev1.LimitRange], namespace string) listerscorev1.LimitRangeNamespaceLister {
	return &limitRangeNamespaceLister{
		kcplisters.NewNamespaced(indexer, namespace),
	}
}

// NewLimitRangeLister returns a new listerscorev1.LimitRangeLister.
// We assume that the indexer:
// - is fed by a workspace-scoped LIST+WATCH
// - uses cache.MetaNamespaceKeyFunc as the key function
// - has the cache.NamespaceIndex as an index
func NewLimitRangeLister(indexer cache.Indexer) listerscorev1.LimitRangeLister {
	return &limitRangeScopedLister{
		listers.New[*corev1.LimitRange](indexer, corev1.Resource("limitrange")),
		indexer,
	}
}
...
// LimitRanges returns an object that can list and get LimitRanges in one namespace.
func (l *limitRangeScopedLister) LimitRanges(namespace string) listerscorev1.LimitRangeNamespaceLister {
