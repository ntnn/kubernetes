...
func (e *TokensController) syncSecret(ctx context.Context) {
	...
	secret, err := e.getSecret(secretInfo.clusterName, secretInfo.namespace, secretInfo.name, secretInfo.uid)
	switch {
	case err != nil:
...
func (e *TokensController) getSecret(clusterName logicalcluster.Name, ns string, name string, uid types.UID) (*v1.Secret, error) {
	// Look up in cache
