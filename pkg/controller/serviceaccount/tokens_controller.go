...
type TokensController struct {
	...
	secrets         kcpcorev1listers.SecretClusterLister
...
func (e *TokensController) syncServiceAccount(ctx context.Context) {
	...
	sa, err := e.getServiceAccount(ctx, saInfo.clusterName, saInfo.namespace, saInfo.name, saInfo.uid, false)
	switch {
	case err != nil:
...
func (e *TokensController) syncSecret(ctx context.Context) {
	...
	secret, err := e.getSecret(ctx, secretInfo.clusterName, secretInfo.namespace, secretInfo.name, secretInfo.uid)
	switch {
	case err != nil:
	...
	case secret == nil:
		...
		if sa, saErr := e.getServiceAccount(ctx, secretInfo.clusterName, secretInfo.namespace, secretInfo.saName, secretInfo.saUID, false); saErr == nil && sa != nil {
			...
			if err := clientretry.RetryOnConflict(RemoveTokenBackoff, func() error {
				return e.removeSecretReference(ctx, secretInfo.clusterName, secretInfo.namespace, secretInfo.saName, secretInfo.saUID, secretInfo.name)
			}); err != nil {
	...
	default:
		...
		sa, saErr := e.getServiceAccount(ctx, secretInfo.clusterName, secretInfo.namespace, secretInfo.saName, secretInfo.saUID, true)
		switch {
		case saErr != nil:
		...
		case sa == nil:
			...
			if retriable, err := e.deleteToken(ctx, secretInfo.clusterName, secretInfo.namespace, secretInfo.name, secretInfo.uid); err != nil {
...
func (e *TokensController) deleteTokens(ctx context.Context, serviceAccount *v1.ServiceAccount) ( /*retry*/ bool, error) {
	...
	for _, token := range tokens {
		r, err := e.deleteToken(ctx, logicalcluster.From(token), token.Namespace, token.Name, token.UID)
		if err != nil {
...
func (e *TokensController) deleteToken(ctx context.Context, clusterName logicalcluster.Name, ns, name string, uid types.UID) ( /*retry*/ bool, error) {
	var opts metav1.DeleteOptions
	if len(uid) > 0 {
		opts.Preconditions = &metav1.Preconditions{UID: &uid}
	}
	err := e.client.Cluster(clusterName.Path()).CoreV1().Secrets(ns).Delete(ctx, name, opts)
...
func (e *TokensController) generateTokenIfNeeded(ctx context.Context, serviceAccount *v1.ServiceAccount, cachedSecret *v1.Secret) ( /* retry */ bool, error) {
	...
	if liveSecret.ResourceVersion != cachedSecret.ResourceVersion {
		...
		klog.FromContext(ctx).V(2).Info("Secret is not up to date, skipping token population", "secret", klog.KRef(liveSecret.Namespace, liveSecret.Name), "cluster", clusterName)
		return false, nil
	}
...
func (e *TokensController) removeSecretReference(ctx context.Context, saClusterName logicalcluster.Name, saNamespace string, saName string, saUID types.UID, secretName string) error {
	// We don't want to update the cache's copy of the service account
...
func (e *TokensController) getServiceAccount(ctx context.Context, clusterName logicalcluster.Name, ns string, name string, uid types.UID, fetchOnCacheMiss bool) (*v1.ServiceAccount, error) {
	// Look up in cache
	...
	// Live lookup
	sa, err = e.client.Cluster(clusterName.Path()).CoreV1().ServiceAccounts(ns).Get(ctx, name, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		return nil, nil
	}
	if err != nil {
...
func (e *TokensController) getSecret(ctx context.Context, clusterName logicalcluster.Name, ns string, name string, uid types.UID) (*v1.Secret, error) {
	// Look up in cache
	secret, err := e.secrets.Cluster(clusterName).Secrets(ns).Get(name)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return nil, nil
		}
...
func (e *TokensController) listTokenSecrets(serviceAccount *v1.ServiceAccount) ([]*v1.Secret, error) {
	namespaceSecrets, err := e.secrets.Cluster(logicalcluster.From(serviceAccount)).Secrets(serviceAccount.Namespace).List(labels.Everything())
	if err != nil {
		return nil, err
	}
