...
func (c *ClusterRoleAggregationController) syncClusterRole(ctx context.Context, key string) error {
	...
	return c.applyClusterRoles(ctx, logicalcluster.From(sharedClusterRole).Path(), sharedClusterRole.Name, newPolicyRules)
}

func (c *ClusterRoleAggregationController) applyClusterRoles(ctx context.Context, clusterPath logicalcluster.Path, name string, newPolicyRules []rbacv1.PolicyRule) error {
	...
	_, err := c.clusterRoleClient.ClusterRoles().Cluster(clusterPath).Apply(ctx, clusterRoleApply, opts)
	return err
}
