...
func (c *ClusterRoleAggregationController) syncClusterRole(ctx context.Context, key string) error {
	...
	return c.applyClusterRoles(ctx, sharedClusterRole.Name, logicalcluster.From(sharedClusterRole).Path(), newPolicyRules)
}

func (c *ClusterRoleAggregationController) applyClusterRoles(ctx context.Context, name string, clusterPath logicalcluster.Path, newPolicyRules []rbacv1.PolicyRule) error {
