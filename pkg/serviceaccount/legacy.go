...
func (v *legacyValidator) Validate(ctx context.Context, tokenData string, public *jwt.Claims, private *legacyPrivateClaims) (*apiserverserviceaccount.ServiceAccountInfo, error) {
	...
	if v.lookup {
		...
		secret, err := v.getter.Cluster(private.ClusterName).GetSecret(ctx, namespace, secretName)
		if err != nil {
		...
		serviceAccount, err := v.getter.Cluster(private.ClusterName).GetServiceAccount(ctx, namespace, serviceAccountName)
		if err != nil {
